...
class MySubscribeCallback(SubscribeCallback):
	...
	def status(self, pubnub, status):
		if status.category == PNStatusCategory.PNUnexpectedDisconnectCategory:
			...
			# UI / internal notifications, etchashvalue = hashlib.sha256(preblk.encode()).hexdigest()
	...
	def message(self, pubnub, message):
		if "GAME OVER" in message.message:
			...
			if(type(msgJson) is dict and "TxID" in msgJson):
				if(msgJson.get("Transaction")[0] == "Bob"):
					...
					if(hashvalue == preblkHash):
						...
						# Checks if Bob's move wins him the game
						bobStr = ""
						for i in bobArray:
							bobStr += i
						bob = []
						for s in itertools.permutations(bobStr):
							bob.append(''.join(s))
...
pubnub.subscribe().channels('Channel-mqdvd3hi4').execute()

# Alice starts off the game
fr = open("block0.json", "r")
preblk = fr.read()
fr.close()
...
cond = True
while(cond):
	blockx = json.dumps({"TxID": 1, "Hash": hashvalue, "Nonce": nonce, "Transaction": transaction}, sort_keys = False, indent = 4, separators = (',', ': '))	
